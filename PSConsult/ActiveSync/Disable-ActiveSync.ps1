function Disable-ActiveSync {
    <#
	.SYNOPSIS
		Disable ActiveSync for all mailboxes except a list of mailbox Primary SMTP Addresses
	
	.DESCRIPTION
		Disable ActiveSync for all mailboxes except a list of mailbox Primary SMTP Addresses
	
	.PARAMETER EmailAddress
		The address of the mailbox to disable active sync for.
	...
	.EXAMPLE
        Get-Mailbox -ResultSize unlimited | Disable-ActiveSync -ExceptionListPath c:\scripts\ExceptionList.txt
...
#>
    ...
    process {
        ...
        foreach ($address in $EmailAddress) {
            ...
            try {
                $gms = Set-CasMailbox -identity $address -ActiveSyncEnabled:$False -ErrorAction stop
